...
struct FootstepArray_
{
  ...
  _header_type header;
   ...
   typedef std::vector< ::jsk_footstep_msgs::Footstep_<ContainerAllocator> , typename std::allocator_traits<ContainerAllocator>::template rebind_alloc< ::jsk_footstep_msgs::Footstep_<ContainerAllocator> >> _footsteps_type;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::jsk_footstep_msgs::FootstepArray_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.footsteps.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::jsk_footstep_msgs::Footstep_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.footsteps[i]);
    }
    if (v.footsteps.empty() || false)
      s << "]";
  }
