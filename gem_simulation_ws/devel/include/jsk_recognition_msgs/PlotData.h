...
struct PlotData_
{
  ...
  _header_type header;
   ...
   typedef std::vector<float, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<float>> _xs_type;
  _xs_type xs;

   typedef std::vector<float, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<float>> _ys_type;
  _ys_type ys;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::jsk_recognition_msgs::PlotData_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.xs.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.xs[i]);
    }
    if (v.xs.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.ys.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.ys[i]);
    }
    if (v.ys.empty() || true)
      s << "]";
    if (true || !indent.empty())
