...
struct Printer< ::jsk_recognition_msgs::Object_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::jsk_recognition_msgs::Object_<ContainerAllocator>& v)
  {
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "image_resources: ";
    if (v.image_resources.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.image_resources.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.image_resources[i]);
    }
    if (v.image_resources.empty() || true)
      s << "]";
    if (true || !indent.empty())
