...
struct Histogram_
{
  ...
  _header_type header;
   ...
   typedef std::vector<double, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<double>> _histogram_type;
...
bool operator==(const ::jsk_recognition_msgs::Histogram_<ContainerAllocator1> & lhs, const ::jsk_recognition_msgs::Histogram_<ContainerAllocator2> & rhs)
{
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::jsk_recognition_msgs::Histogram_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.histogram.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.histogram[i]);
    }
    if (v.histogram.empty() || true)
      s << "]";
  }
