...
struct WeightedPoseArray_
{
  ...
  _header_type header;
   ...
   typedef std::vector<float, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<float>> _weights_type;
...
bool operator==(const ::jsk_recognition_msgs::WeightedPoseArray_<ContainerAllocator1> & lhs, const ::jsk_recognition_msgs::WeightedPoseArray_<ContainerAllocator2> & rhs)
{
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::jsk_recognition_msgs::WeightedPoseArray_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.weights.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.weights[i]);
    }
    if (v.weights.empty() || true)
      s << "]";
    if (true || !indent.empty())
