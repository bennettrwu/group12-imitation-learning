...
struct ColorHistogram_
{
  ...
  _header_type header;
   ...
   typedef std::vector<float, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<float>> _histogram_type;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::jsk_recognition_msgs::ColorHistogram_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.histogram.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.histogram[i]);
    }
    if (v.histogram.empty() || true)
      s << "]";
  }
