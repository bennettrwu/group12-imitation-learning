...
struct BoundingBoxArray_
{
  ...
  _header_type header;
   ...
   typedef std::vector< ::jsk_recognition_msgs::BoundingBox_<ContainerAllocator> , typename std::allocator_traits<ContainerAllocator>::template rebind_alloc< ::jsk_recognition_msgs::BoundingBox_<ContainerAllocator> >> _boxes_type;
  _boxes_type boxes;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::jsk_recognition_msgs::BoundingBoxArray_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.boxes.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::jsk_recognition_msgs::BoundingBox_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.boxes[i]);
    }
    if (v.boxes.empty() || false)
      s << "]";
  }
