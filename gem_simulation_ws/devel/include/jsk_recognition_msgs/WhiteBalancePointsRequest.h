...
bool operator==(const ::jsk_recognition_msgs::WhiteBalancePointsRequest_<ContainerAllocator1> & lhs, const ::jsk_recognition_msgs::WhiteBalancePointsRequest_<ContainerAllocator2> & rhs)
{
  return lhs.reference_color == rhs.reference_color &&
    lhs.input == rhs.input;
}
...
struct Printer< ::jsk_recognition_msgs::WhiteBalancePointsRequest_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::jsk_recognition_msgs::WhiteBalancePointsRequest_<ContainerAllocator>& v)
  {
    if (false || !indent.empty())
      s << std::endl;
    s << indent << "reference_color: ";
    if (v.reference_color.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.reference_color.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.reference_color[i]);
    }
    if (v.reference_color.empty() || true)
      s << "]";
