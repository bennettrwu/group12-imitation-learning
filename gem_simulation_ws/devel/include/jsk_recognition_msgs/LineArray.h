...
struct LineArray_
{
  ...
  _header_type header;
   ...
   typedef std::vector< ::jsk_recognition_msgs::Line_<ContainerAllocator> , typename std::allocator_traits<ContainerAllocator>::template rebind_alloc< ::jsk_recognition_msgs::Line_<ContainerAllocator> >> _lines_type;
  _lines_type lines;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::jsk_recognition_msgs::LineArray_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.lines.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::jsk_recognition_msgs::Line_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.lines[i]);
    }
    if (v.lines.empty() || false)
      s << "]";
  }
