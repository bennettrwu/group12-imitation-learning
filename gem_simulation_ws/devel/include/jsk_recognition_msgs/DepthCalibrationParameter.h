...
struct Printer< ::jsk_recognition_msgs::DepthCalibrationParameter_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::jsk_recognition_msgs::DepthCalibrationParameter_<ContainerAllocator>& v)
  {
    if (false || !indent.empty())
      s << std::endl;
    s << indent << "coefficients2: ";
    if (v.coefficients2.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.coefficients2.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.coefficients2[i]);
    }
    if (v.coefficients2.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "coefficients1: ";
    if (v.coefficients1.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.coefficients1.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.coefficients1[i]);
    }
    if (v.coefficients1.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "coefficients0: ";
    if (v.coefficients0.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.coefficients0.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.coefficients0[i]);
    }
    if (v.coefficients0.empty() || true)
      s << "]";
    if (true || !indent.empty())
