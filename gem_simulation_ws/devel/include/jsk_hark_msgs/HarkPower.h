...
struct HarkPower_
{
  ...
  _data_bytes_type data_bytes;
   ...
   typedef std::vector<float, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<float>> _powers_type;
  _powers_type powers;
...
bool operator==(const ::jsk_hark_msgs::HarkPower_<ContainerAllocator1> & lhs, const ::jsk_hark_msgs::HarkPower_<ContainerAllocator2> & rhs)
{
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::jsk_hark_msgs::HarkPower_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.powers.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.powers[i]);
    }
    if (v.powers.empty() || true)
      s << "]";
  }
