...
struct MultiTouch_
{
  ...
  (void)_alloc;
   ...
   typedef std::vector< ::jsk_gui_msgs::Touch_<ContainerAllocator> , typename std::allocator_traits<ContainerAllocator>::template rebind_alloc< ::jsk_gui_msgs::Touch_<ContainerAllocator> >> _touches_type;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::jsk_gui_msgs::MultiTouch_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.touches.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::jsk_gui_msgs::Touch_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.touches[i]);
    }
    if (v.touches.empty() || false)
      s << "]";
  }
